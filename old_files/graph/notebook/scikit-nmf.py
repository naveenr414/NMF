...
SAMPLES = 10000
CANCER_TYPES = 5 # How many of the types of cancer should we use
normalize = False
outliers = True
...
def _multiplicative_update_h(X, W, H, beta_loss, l1_reg_H, l2_reg_H, gamma,D=0,weights=0,lamb=0):
    """update H in Multiplicative Update NMF"""
    if beta_loss == 2:
        numerator = safe_sparse_dot(W.T, X)+lamb*(H.dot(weights.T))
        denominator = np.dot(np.dot(W.T, W), H) + lamb*(H.dot(D.T))
...
def _fit_multiplicative_update(X, W, H, beta_loss='frobenius',
                               ...
                               update_H=True, verbose=0,weights=0,lamb=0):
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        if update_H:
            delta_H = _multiplicative_update_h(X, W, H, beta_loss, l1_reg_H,
                                               l2_reg_H, gamma,D=D,weights=weights,lamb=lamb)
            H *= delta_H
...
def non_negative_factorization(X, W=None, H=None, n_components=None,
                               ...
                               verbose=0, shuffle=False,weights=0,lamb=0):
...
class NMF(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, X, y=None, W=None, H=None,lamb=0,weights=0):
        ...
        W, H, n_iter_ = non_negative_factorization(
            ...
            shuffle=self.shuffle,weights=weights,lamb=lamb)
    ...
    def transform(self, X,weights=0,lamb=0):
...
for i in range(100):
    model = NMF(n_components=10,init='random',random_state=int(np.random.random()*1000000))
    model.solver = 'mu'
    W = model.fit_transform(allData.T)
    H = model.components_
    W,H = H.T,W.T
    print(np.linalg.norm(allData-W.dot(H)))

for k in range(5):
    for i in range(0,20):
        model = NMF(n_components=10,init='random',random_state=0)
        model.solver = 'mu'
        W = model.fit_transform(allData.T,lamb=i,weights=createNearestNeighbor(allData,k=k))
        H = model.components_
        W,H = H.T,W.T
        print(np.linalg.norm(allData-W.dot(H)),i,k)

L = np.zeros((allData.shape[0],allData.shape[0]))
U,V,r = GNMF(allData.T,L,n_components=2)
print(np.linalg.norm(V.dot(U.T)-allData))
