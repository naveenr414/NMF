...
z = glob.glob("./runs/run_{}/tmp*".format(str(run_number)))
...
score_list = []
settings_list = ["Lambda","Rank","K"]

create_signatures = True

for i in z:
    f = open(i).read().split("\n")[1:-1]
    settings = [float(j) for j in f[6].split(" ")]
    score = float(f[7].split(": ")[1])

    if create_signatures:
        exec("signature="+f[8].replace("Signatures: ",""))
        name = str(score)+"_"+"_".join([str(j) for j in settings])+".jpg"
        sbs_signature_plot(pd.DataFrame(signature,columns=mutations))
        plt.savefig("signatures/signatures_{}/{}".format(run_number,name))
        plt.close()

    d = {}
    for i in range(len(settings_list)):
        d[settings_list[i]] = settings[i]
...
average_score = sum([i[0] for i in score_list])/len(score_list)
print(average_score)
